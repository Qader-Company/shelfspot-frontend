...
export function CategoryPage() {
  ...
  const rows = useMemo<CategoryRow[]>(() => (categories.data?.data ?? []).filter((item) => matchesCategoryName(item, search.trim())).map((item) => ({ id: String(item.id), name: nameOf(item, locale), thumbnailAlt: nameOf(item, locale), brand: relation(item.brand, item.brand_name), subBrand: relation(item.sub_brand, item.sub_brand_name), isActive: activeOf(item), statusDisplay: "toggle", createdDate: item.created_at ? new Intl.DateTimeFormat(locale, { dateStyle: "medium", timeStyle: "short" }).format(new Date(item.created_at)) : "-" })), [categories.data?.data, locale, search]);
