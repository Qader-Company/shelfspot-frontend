...
type SubCategoryDialog = "add" | "edit" | "delete" | "import" | null;

export function SubCategoryPage() {
  const t = useTranslations("dashboard");
  const [openDialog, setOpenDialog] = useState<SubCategoryDialog>(null);
  const close = () => setOpenDialog(null);
  const handleDelete = () => setOpenDialog("delete");
  const handleEdit = () => setOpenDialog("edit");

  const extraColumns: CatalogExtraColumn[] = [
    { key: "brand",    header: t("catalogPage.subCategory.table.columns.brand"),    getValue: (r) => String(r.brand ?? "") },
    { key: "subBrand", header: t("catalogPage.subCategory.table.columns.subBrand"), getValue: (r) => String(r.subBrand ?? "") },
    { key: "category", header: t("catalogPage.subCategory.table.columns.category"), getValue: (r) => String(r.category ?? "") },
  ];

  const tableLabels = {
    nameColumn:    t("catalogPage.subCategory.table.columns.subCategory"),
  ...
  return (
    <div className="space-y-6 px-4 py-8 lg:px-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          <h1 className="text-3xl font-bold leading-tight text-foreground">{t("catalogPage.subCategory.title")}</h1>
          <p className="mt-2 text-lg font-medium text-muted-foreground">{t("catalogPage.subCategory.subtitle")}</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button type="button" className="h-10 gap-2 rounded-lg px-4 text-sm font-semibold text-white hover:text-white" onClick={() => setOpenDialog("add")}>
            <AddIcon className="size-4" />{t("catalogPage.subCategory.actions.add")}
          </Button>
        </div>
      </div>
      ...
      <CatalogItemsTable rows={subCategoryRows} labels={tableLabels} extraColumns={extraColumns} onDelete={handleDelete} onEdit={handleEdit} />
      ...
      <DeleteConfirmDialog
        ...
        title={t("catalogPage.subCategory.deleteDialog.title")}
        descriptionLine1={t("catalogPage.subCategory.deleteDialog.descriptionLine1")}
        descriptionLine2={t("catalogPage.subCategory.deleteDialog.descriptionLine2")}
        cancelLabel={t("catalogPage.subCategory.deleteDialog.cancel")}
        confirmLabel={t("catalogPage.subCategory.deleteDialog.confirm")}
        onClose={close}
      />

      <CatalogFormDialog isOpen={openDialog === "add" || openDialog === "edit"} title={openDialog === "edit" ? t("catalogPage.subCategory.dialog.editTitle") : t("catalogPage.subCategory.dialog.title")} closeLabel={t("catalogPage.dialog.close")} cancelLabel={t("catalogPage.dialog.cancel")} saveLabel={t("catalogPage.dialog.save")} onClose={close}>
        ...
        <CatalogSelectField label={t("catalogPage.subCategory.dialog.categoryLabel")} placeholder={t("catalogPage.subCategory.dialog.selectCategory")} options={catalogCategoryOptions} />
        <CatalogUploadArea label={t("catalogPage.subCategory.dialog.uploadLabel")} hint={t("catalogPage.dialog.uploadHint")} />
        <div className="space-y-1.5">
          <label className="text-sm font-semibold text-foreground">{t("catalogPage.subCategory.dialog.nameLabel")}</label>
          <Input type="text" placeholder={t("catalogPage.subCategory.dialog.namePlaceholder")} className="h-11 rounded-lg border-border bg-secondary text-sm shadow-none" />
        </div>
        <div className="space-y-1.5">
          <p className="text-sm font-semibold text-foreground">{t("catalogPage.dialog.statusLabel")}</p>
          <CatalogStatusField activeLabel={t("catalogPage.dialog.statusActive")} description={t("catalogPage.subCategory.dialog.statusDescription")} ariaLabel={t("catalogPage.dialog.statusActive")} />
        </div>
      </CatalogFormDialog>
