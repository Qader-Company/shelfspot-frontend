...
export async function getWallet(params: Record<string, unknown>): Promise<WalletData> {
  ...
  const transactionSource = Array.isArray(first.data) ? first.data : Array.isArray(payload.transactions) ? payload.transactions : Array.isArray(first.transactions) ? first.transactions : Array.isArray(root.transactions) ? root.transactions : [];
