...
export interface ProductPayload { nameEn: string; nameAr: string; descriptionEn: string; descriptionAr: string; sku: string; barcode: string; brandId: string; subBrandId: string; categoryId: string; subCategoryId: string; isActive: boolean; image?: File; }
function form(payload: ProductPayload, update = false) { const data = new FormData(); data.append("translations[en][name]", payload.nameEn); data.append("translations[ar][name]", payload.nameAr); data.append("translations[en][description]", payload.descriptionEn); data.append("translations[ar][description]", payload.descriptionAr); data.append("sku", payload.sku); data.append("barcode", payload.barcode); data.append("brand_id", payload.brandId); data.append("sub_brand_id", payload.subBrandId); data.append("category_id", payload.categoryId); data.append("sub_category_id", payload.subCategoryId); data.append("is_active", payload.isActive ? "1" : "0"); if (update) data.append("_method", "put"); if (payload.image) data.append("image", payload.image, payload.image.name); return data; }
