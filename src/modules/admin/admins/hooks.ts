...
export function useAccessControl() {
  ...
  return { admins: useQuery({ queryKey: keys.admins, queryFn: () => api.getAdmins({ per_page: 100 }) }), roles: useQuery({ queryKey: keys.roles, queryFn: () => api.getRoles({ per_page: 100 }) }), permissions: useQuery({ queryKey: keys.permissions, queryFn: api.getPermissions }),
