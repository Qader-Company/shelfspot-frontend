...
import { useCategoriesQuery, useCreateCategoryMutation, useDeleteCategoryMutation, useUpdateCategoryMutation } from "@/modules/company/catalog/categories/hooks";
import { useSubBrandsQuery } from "@/modules/company/catalog/sub-brands/use-query";
import { downloadCategoriesTemplateService, importCategoriesService } from "@/modules/company/catalog/categories/service";
import type { CompanyCategory } from "@/modules/company/catalog/categories/types";
...
export function CategoryPage() {
  ...
  const rows = useMemo<CategoryRow[]>(() => (categories.data?.data ?? []).filter((item) => matchesCategoryName(item, search.trim())).map((item) => ({ id: String(item.id), name: nameOf(item, locale), thumbnailAlt: nameOf(item, locale), thumbnailUrl: item.image_url ?? item.image ?? item.logo_url ?? item.logo ?? null, brand: relation(item.brand, item.brand_name), subBrand: relation(item.sub_brand, item.sub_brand_name), isActive: activeOf(item), statusDisplay: "toggle", createdDate: item.created_at ? new Intl.DateTimeFormat(locale, { dateStyle: "medium", timeStyle: "short" }).format(new Date(item.created_at)) : "-" })), [categories.data?.data, locale, search]);
  ...
  const openAdd = () => { setBrandId(""); setSubBrandId(""); setNameEn(""); setNameAr(""); setActive(true); setImage(null); setExistingImage(null); setError(""); setDialog("add"); };
  async function save() { if (!brandId || !subBrandId || !nameEn.trim() || !nameAr.trim() || (dialog === "add" && !image)) { setError(t("catalogPage.category.dialog.requiredFields")); return; } setError(""); try { const payload = { brandId, subBrandId, nameEn: nameEn.trim(), nameAr: nameAr.trim(), isActive: active, image: image ?? undefined, ...(dialog === "edit" ? { logoAction: image ? "replace" as const : existingImage ? "keep" as const : "remove" as const } : {}) }; const response = dialog === "edit" && selectedId ? await updateMutation.mutateAsync({ id: selectedId, payload }) : await createMutation.mutateAsync(payload); setSuccess(response.message); await refresh(); close(); } catch (value) { setError(normalizeApiError(value).message); } }
  function edit(id: string) { const item = categories.data?.data.find((x) => String(x.id) === id); if (!item) return; setSelectedId(id); setBrandId(String(item.brand_id)); setSubBrandId(String(item.sub_brand_id)); setNameEn(editName(item, "en")); setNameAr(editName(item, "ar")); setActive(activeOf(item)); setImage(null); setExistingImage(item.image_url ?? item.image ?? item.logo_url ?? item.logo ?? null); setDialog("edit"); }
  ...
  const labels = { nameColumn: t("catalogPage.category.table.columns.category"), status: t("catalogPage.table.columns.status"), createdDate: t("catalogPage.table.columns.createdDate"), action: t("catalogPage.table.columns.action"), selectAll: t("catalogPage.table.actions.selectAll"), selectRow: t("catalogPage.table.actions.selectRow"), delete: t("catalogPage.table.actions.delete"), edit: t("catalogPage.table.actions.edit"), toggleStatus: t("catalogPage.table.actions.toggleStatus"), activeLabel: t("catalogPage.status.active"), inactiveLabel: t("catalogPage.status.inactive"), loading: t("catalogPage.table.loading"), empty: t("catalogPage.table.empty") };
  ...
  return <div className="space-y-6 px-4 py-8 lg:px-8">
    <div className="flex justify-between gap-4"><div><h1 className="text-3xl font-bold">{t("catalogPage.category.title")}</h1><p className="mt-2 text-lg text-muted-foreground">{t("catalogPage.category.subtitle")}</p></div><PermissionGate permission="create_category"><div className="flex gap-3"><Button variant="outline" onClick={() => { setImportFile(null); setImportError(""); setDialog("import"); }}><UploadIcon className="size-4" />{t("catalogPage.actions.import")}</Button><Button onClick={openAdd}><AddIcon className="size-4" />{t("catalogPage.category.actions.add")}</Button></div></PermissionGate></div>
    ...
    {categories.isError ? <p className="rounded-xl border border-destructive/20 bg-destructive/10 px-4 py-3 text-destructive">{normalizeApiError(categories.error).message}</p> : <div className={cn(categories.isFetching && "opacity-60")}><CatalogItemsTable rows={rows} labels={labels} extraColumns={extra} isLoading={categories.isLoading} onDelete={(id) => { setSelectedId(id); setDialog("delete"); }} onEdit={edit} /></div>}
    ...
    <CatalogFormDialog isOpen={dialog === "add" || dialog === "edit"} title={dialog === "edit" ? t("catalogPage.category.dialog.editTitle") : t("catalogPage.category.dialog.title")} closeLabel={t("catalogPage.dialog.close")} cancelLabel={t("catalogPage.dialog.cancel")} saveLabel={t("catalogPage.dialog.save")} onClose={close} onSubmit={save} isPending={createMutation.isPending || updateMutation.isPending} errorMessage={error}>
      ...
      <CatalogUploadArea label={t("catalogPage.category.dialog.uploadLabel")} hint={t("catalogPage.dialog.uploadHint")} file={image} onFileChange={setImage} existingImageUrl={dialog === "edit" ? existingImage : undefined} onRemove={dialog === "edit" ? () => { setImage(null); setExistingImage(null); } : undefined} />
