...
export function SubBrandPage() {
  ...
  return <div className="space-y-6 px-4 py-8 lg:px-8">
    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div><h1 className="text-3xl font-bold">{t("catalogPage.subBrand.title")}</h1><p className="mt-2 text-lg text-muted-foreground">{t("catalogPage.subBrand.subtitle")}</p></div><PermissionGate permission="create_sub_brand"><div className="flex gap-3"><Button variant="outline" onClick={() => { setImportFile(null); setImportError(""); setOpenDialog("import"); }}><UploadIcon className="size-4" />{t("catalogPage.actions.import")}</Button><Button onClick={openAdd}><AddIcon className="size-4" />{t("catalogPage.subBrand.actions.add")}</Button></div></PermissionGate></div>
