...
import { useCategoriesQuery } from "@/modules/company/dashboard/hooks/use-categories";
import { useSubBrandsQuery } from "@/modules/company/dashboard/hooks/use-sub-brands-query";
import { useCreateSubCategoryMutation, useDeleteSubCategoryMutation, useSubCategoriesQuery, useUpdateSubCategoryMutation } from "@/modules/company/dashboard/hooks/use-sub-categories";
import { downloadSubCategoriesTemplateService, importSubCategoriesService } from "@/modules/company/dashboard/services/sub-category-services";
import type { CompanySubCategory } from "@/modules/company/dashboard/types/sub-category";
...
import type { SubCategoryRow } from "./catalog.seed";
...
export function SubCategoryPage() {
  ...
  return <div className="space-y-6 px-4 py-8 lg:px-8">
    ...
    {list.isError ? <p className="rounded-xl border border-destructive/20 bg-destructive/10 px-4 py-3 text-destructive">{normalizeApiError(list.error).message}</p> : <div className={cn(list.isFetching && "opacity-60")}><CatalogItemsTable rows={rows} labels={labels} extraColumns={extra} onDelete={(id) => { setSelectedId(id); setDialog("delete"); }} onEdit={edit} /></div>}
