...
export function SubCategoryPage() {
  ...
  return <div className="space-y-6 px-4 py-8 lg:px-8">
    <div className="flex justify-between"><div><h1 className="text-3xl font-bold">{t("catalogPage.subCategory.title")}</h1><p className="mt-2 text-lg text-muted-foreground">{t("catalogPage.subCategory.subtitle")}</p></div><PermissionGate permission="create_sub_category"><div className="flex gap-3"><Button variant="outline" onClick={() => { setImportFile(null); setImportError(""); setDialog("import"); }}><UploadIcon className="size-4" />{t("catalogPage.actions.import")}</Button><Button onClick={add}><AddIcon className="size-4" />{t("catalogPage.subCategory.actions.add")}</Button></div></PermissionGate></div>
