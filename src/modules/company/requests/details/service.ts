...
export async function updateTask(id: number | string, payload: FormData | Record<string, unknown>) { return (await apiClient.patch<TaskResponse>(`${endpoint}/${id}`, payload, payload instanceof FormData ? { headers: { "Content-Type": undefined } } : undefined)).data; }
