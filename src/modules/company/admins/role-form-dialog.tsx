...
import type { PermissionGroup, Role } from "./access-control-api";

interface Props { isOpen: boolean; role?: Role; permissions: PermissionGroup[]; labels: Record<string, string>; onClose: () => void; onSubmit: (payload: Record<string, unknown>) => Promise<unknown>; }
...
function RoleFormContent({ role, permissions, labels, onClose, onSubmit }: Omit<Props, "isOpen">) {
  const [name, setName] = useState(role?.name ?? ""); const [active, setActive] = useState(Boolean(role?.active ?? role?.is_active ?? true)); const [selected, setSelected] = useState<string[]>(role?.permissions?.map(permission => String(permission.id)) ?? []); const [pending, setPending] = useState(false); const [error, setError] = useState("");
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 p-4"><section role="dialog" aria-modal="true" className="max-h-[90dvh] w-full max-w-2xl overflow-y-auto rounded-2xl border bg-card p-5 shadow-xl sm:p-8"><h2 className="text-center text-2xl font-bold">{labels.title}</h2><form onSubmit={submit}><div className="mt-6 space-y-5">
    ...
    <div><p className="mb-3 text-sm font-medium">{labels.permission}</p><div className="space-y-5">{permissions.map(group => <fieldset key={group.key} className="rounded-xl border border-border p-4"><legend className="px-2 text-sm font-semibold">{group.label}</legend><div className="grid gap-3 sm:grid-cols-2">{group.permissions.map(permission => { const permissionId = String(permission.id); return <label key={permission.id} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={selected.includes(permissionId)} onChange={() => setSelected(current => current.includes(permissionId) ? current.filter(id => id !== permissionId) : [...current, permissionId])}/><span>{permission.label ?? permission.name}</span></label>; })}</div></fieldset>)}</div></div>
