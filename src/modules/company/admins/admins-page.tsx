...
export function AdminsPage() {
  ...
  const [tab, setTab] = useState<Tab>("admins"); const [dialog, setDialog] = useState<Dialog>(null); const [search, setSearch] = useState("");
  const [selectedAdmin, setSelectedAdmin] = useState<Admin>(); const [selectedRole, setSelectedRole] = useState<Role>();
  const admins = useAdmins({ per_page: 100 }); const roles = useRoles({ per_page: 100 }); const permissions = usePermissions();
