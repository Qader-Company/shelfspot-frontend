...
export function AdminsPage() {
  ...
  const adminRows = useMemo(() => (admins.data?.items ?? []).map(a => { const roleName = typeof a.roles === "string" ? a.roles : Array.isArray(a.roles) ? a.roles.join(", ") : typeof a.roles === "object" && a.roles ? a.roles.name : typeof a.role === "object" ? a.role.name : a.role ?? "-"; return { admin: a, roleName }; }).filter(({ admin, roleName }) => `${admin.name} ${admin.email} ${admin.phone ?? admin.phone_number ?? ""} ${roleName}`.toLowerCase().includes(search.toLowerCase())).map(({ admin, roleName }) => ({ id: String(admin.id), name: admin.name, phone: admin.phone ?? admin.phone_number ?? "-", email: admin.email, role: roleName, isActive: activeOf(admin as Admin & { is_active?: boolean | number }), isProtected: isProtectedAdmin(admin) })), [admins.data, search]);
