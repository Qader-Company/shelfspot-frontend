...
export async function verifyPasswordResetOtp(
  ...
  const response = await client.post<unknown>(
    getAuthContextConfig(context).verifyResetOtpEndpoint,
    { email: payload.email, otp: payload.code },
  );
...
export async function resetPassword(
  context: AuthContext,
  payload: { password: string; confirmPassword: string; token: string },
) {
  const client = context === "admin" ? adminApiClient : apiClient;
  await client.post(
    getAuthContextConfig(context).resetPasswordEndpoint,
    {
